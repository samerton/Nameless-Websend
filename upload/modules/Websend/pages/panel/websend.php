...
if(isset($_GET['hook'])){
	...
	if(Input::exists()){
		if(Token::check(Input::get('token'))){
            ...
            // Adding it to the hook system if it isn't in there yet
            if (!in_array('WSHook::execute', HookHandler::getHook($_GET['hook'])['hooks'] != null ? HookHandler::getHook($_GET['hook'])['hooks'] : [])) {
                HookHandler::registerHook($_GET['hook'], 'WSHook::execute');
            }
	...
	if(Input::exists()){
		if(Token::check(Input::get('token'))){
            ...
            WSHook::setWSHost($adress);
            WSHook::setWSPort($port);
            WSHook::setWSPassword($password);
	...
	if(!isset($ws_config) && file_exists(ROOT_PATH . '/modules/Websend/config.php')){
		require(ROOT_PATH . '/modules/Websend/config.php');
	}
